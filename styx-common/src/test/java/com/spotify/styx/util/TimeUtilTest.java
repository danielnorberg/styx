...
public class TimeUtilTest {
  ...
  @Test
  public void shouldReturnLastInstantUnchangedIfMatchingPartitioningTime() throws Exception {
    final Instant lastTimeHours = Instant.parse("2016-01-19T09:00:00.00Z");
    final Instant lastTimeDays = Instant.parse("2016-01-19T00:00:00.00Z");
    final Instant lastTimeWeeks = Instant.parse("2016-01-18T00:00:00.00Z");
    final Instant lastTimeMonths = Instant.parse("2016-01-01T00:00:00.00Z");

    final Instant hour = lastInstant(lastTimeHours, Partitioning.HOURS);
    assertThat(hour, is(lastTimeHours));

    final Instant day = lastInstant(lastTimeDays, Partitioning.DAYS);
    assertThat(day, is(lastTimeDays));

    final Instant weeks = lastInstant(lastTimeWeeks, Partitioning.WEEKS);
    assertThat(weeks, is(lastTimeWeeks));

    final Instant months = lastInstant(lastTimeMonths, Partitioning.MONTHS);
    assertThat(months, is(lastTimeMonths));
  }
  ...
  public void shouldTestWellKnownAlignedInstants() throws Exception {
    assertTrue(isAligned(Instant.parse("2017-02-06T10:00:00.00Z"), Partitioning.HOURS));
    assertTrue(isAligned(Instant.parse("2017-02-06T00:00:00.00Z"), Partitioning.DAYS));
    assertTrue(isAligned(Instant.parse("2017-02-06T00:00:00.00Z"), Partitioning.WEEKS));
    assertTrue(isAligned(Instant.parse("2017-02-01T00:00:00.00Z"), Partitioning.MONTHS));
    assertTrue(isAligned(Instant.parse("2017-01-01T00:00:00.00Z"), Partitioning.YEARS));

    assertFalse(isAligned(Instant.parse("2017-02-06T10:01:00.00Z"), Partitioning.HOURS));
    assertFalse(isAligned(Instant.parse("2017-02-06T01:00:00.00Z"), Partitioning.DAYS));
    assertFalse(isAligned(Instant.parse("2017-02-07T00:00:00.00Z"), Partitioning.WEEKS));
    assertFalse(isAligned(Instant.parse("2017-02-02T00:00:00.00Z"), Partitioning.MONTHS));
    assertFalse(isAligned(Instant.parse("2017-01-02T00:00:00.00Z"), Partitioning.YEARS));
  }

  @Test
  public void shouldTestCustomAlignedInstants() throws Exception {
    Partitioning custom = Partitioning.parse("15,42 10 * * *");
  ...
  @Test
  public void cronPartitioningShouldNotEqualEquivalentWellKnownPartitioning() throws Exception {
    assertFalse(Partitioning.parse("0 * * * *").equals(Partitioning.HOURS));
  }
}
